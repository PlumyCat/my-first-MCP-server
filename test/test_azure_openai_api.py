...
async def run_azure_tests():
    ...
    try:
        ...
        print(f"   📊 Temps moyen: {total_time/len(test_questions):.2f}s par test")
...
if __name__ == "__main__":
    ...
    print(f"🔑 Azure API: {'✅ Configuré' if os.getenv('AZURE_OPENAI_API_KEY') else '❌ Manquant'}")
    print(f"🌐 Endpoint: {'✅ Configuré' if os.getenv('AZURE_OPENAI_ENDPOINT') else '❌ Manquant'}")
    print(f"🚀 Deployment: {'✅ Configuré' if os.getenv('AZURE_OPENAI_DEPLOYMENT_NAME') else '❌ Manquant'}")
